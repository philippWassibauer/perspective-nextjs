...
function midOnePlot(container: HTMLSelection, settings: Settings) {
  ...
  if (settings.splitValues.length !== 0) {
    let xyGrid = gridLayoutMultiChart().svg(false).elementsPrefix("xy-scatter");
...
midOnePlot.can_render_column_styles = (type: Type, group?: string) => {
  return type === "string" && group === "Symbol";
};
midOnePlot.column_style_controls = (type: Type, group?: string) => {
